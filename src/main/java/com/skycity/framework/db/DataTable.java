...
public class DataTable implements Serializable, Cloneable, Iterable<DataRow> {
			...
			for (int j = i + 1; j < types.length; j++) {
				if (columnName == null) {
					throw new RuntimeException(
							"Column name cann't be null,index is " + i);
				}
				if (columnName.equalsIgnoreCase(types[j].getColumnName())) {
					types[j].ColumnName = (columnName + "_" + String
							.valueOf(++count));
				}
			}
		}
				...
				if (index >= begin) {
					...
					for (int j = 1; j <= columnCount; j++) {
						if (this.columns[(j - 1)].getColumnType() == 10) {
							...
							t[(j - 1)] = (("true".equals(rs.getString(j)))
									|| ("1".equals(rs.getString(j))) ? "1"
									: "0");
	...
	public void insertColumn(DataColumn dc, Object[] columnValue, int index) {
		...
		for (int i = 0; i < this.columns.length; i++) {
			if (this.columns[i].getColumnName().equalsIgnoreCase(
					dc.getColumnName())) {
				throw new RuntimeException("Column is exist:"
						+ dc.getColumnName());
			}
		}
	...
	public void insertRow(Object[] rowValue, int index) {
		...
		if (rowValue != null) {
			...
			if (rowValue.length != this.columns.length) {
				throw new RuntimeException("Parameter's length is "
						+ rowValue.length + "，but column count is "
						+ this.columns.length);
			}
			for (int i = 0; i < this.columns.length; i++) {
				if (this.columns[i].ColumnType == 12) {
					Object v = rowValue[i];
					if ((v != null) && (!Date.class.isInstance(v))) {
	...
	public Mapx<String, Object> toMapx(String keyColumnName,
		...
		for (int i = 0; i < this.columns.length; i++) {
			...
			} else if (this.columns[i].getColumnName().equalsIgnoreCase(
					valueColumnName)) {
				valueIndex = i;
				valueFlag = true;
				if (keyFlag) {
	...
	public Mapx<String, Object> toMapx(int keyColumnIndex, int valueColumnIndex) {
		if ((keyColumnIndex < 0) || (keyColumnIndex >= this.columns.length)) {
			throw new RuntimeException("Key index is out of range:"
					+ keyColumnIndex);
		}
		if ((valueColumnIndex < 0) || (valueColumnIndex >= this.columns.length)) {
			throw new RuntimeException("Value index is out of range:"
					+ valueColumnIndex);
		}
	...
	public void union(DataTable anotherDT) {
		...
		if (getColCount() != anotherDT.getColCount()) {
			throw new RuntimeException("This's column count is "
					+ getColCount()
					+ " ,but parameter's column column count is "
		...
		System.arraycopy(anotherDT.rows, 0, newRows, srcPos,
				anotherDT.getRowCount());
		this.rows = null;
		this.rows = newRows;
	}
	...
	public DataTable getPagedDataTable(int pageSize, int pageIndex) {
		...
		for (int i = pageIndex * pageSize; (i < (pageIndex + 1) * pageSize)
