...
//        		});
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		System.out.println("使用email登陆");
		String email = (String) token.getPrincipal(); // 得到用户名
		User user = userService.querySingleUserByEmail(email);
		if(user==null){
...
//		ExcessiveAttemptsException（登录失败次数过多）
		return new SimpleAuthenticationInfo(user.getUserName(), user.getUserPassword(),ByteSource.Util.bytes(user.getPasswdKey().getBytes()), getName());
	}
